...
import exchangeView from './QuotaView.vue'
...
export default {
    ...
    columns () {
      return [{
        ...
        customRender: (text, row, index) => {
          switch (text) {
            case '0':
              return <a-tag>管理员添加</a-tag>
            case '1':
              return <a-tag>充值</a-tag>
            default:
              return '- -'
          }
        }
  ...
  methods: {
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('/cos/quota-record-info/' + ids).then(() => {
    ...
    fetch (params = {}) {
      ...
      this.$get('/cos/quota-record-info/page', {
        ...params
      }).then((r) => {
